...
from game.pkchess.exception import MoveDestinationOutOfMapError
...
class GameController(ClearableMixin):
    ...
    def player_move(cls, channel_oid: ObjectId, player_oid: ObjectId, x_offset: int, y_offset: int) \
        ...
        if game.is_current_action_done(PlayerAction.MOVE):
            return PlayerActionResult.X_ALREADY_MOVED
        ...
        try:
            if not game.map.player_move(player_oid, x_offset, y_offset):
        ...
        game.record_action_done(PlayerAction.MOVE)
        return PlayerActionResult.O_ACTED

    @classmethod
    def player_skill(cls):
        pass  # TODO: Game - skill (implement & test)
