...
from mongodb.factory.execode import ExecodeManager
from mongodb.factory.channel import ChannelManager
from mongodb.factory.user import RootUserManager
...
class TestExecodeManager(TestModelMixin, TestTimeComparisonMixin, TestDatabaseMixin):
    ...
    def setUpTestClass(cls):
        cls.inst = ExecodeManager()

    def test_enqueue_action(self):
        now = now_utc_aware()
        result = self.inst.enqueue_execode(self.CREATOR_OID, Execode.REGISTER_CHANNEL)
    ...
    def test_enqueue_with_model(self):
        now = now_utc_aware()
        result = self.inst.enqueue_execode(
    ...
    def test_enqueue_no_model_class(self):
        result = self.inst.enqueue_execode(self.CREATOR_OID, Execode.REGISTER_CHANNEL, A="B")
    ...
    def test_enqueue_invalid_model(self):
        result = self.inst.enqueue_execode(self.CREATOR_OID, Execode.AR_ADD, AutoReplyModuleExecodeModel, Keyword="A")
    ...
    def test_enqueue_unknown_action(self):
        result = self.inst.enqueue_execode(self.CREATOR_OID, Execode.UNKNOWN)
    ...
    def test_get_list(self):
        ...
        for _ in range(5):
            mdl = ExecodeEntryModel(CreatorOid=self.CREATOR_OID, Execode=self.inst.generate_hex_token(),
                                    ActionType=Execode.REGISTER_CHANNEL, Timestamp=now_utc_aware(for_mongo=True))
            self.inst.insert_one_model(mdl)
            expected.add(mdl)

        for _ in range(3):
            mdl = ExecodeEntryModel(CreatorOid=self.CREATOR_OID_2, Execode=self.inst.generate_hex_token(),
                                    ActionType=Execode.REGISTER_CHANNEL, Timestamp=now_utc_aware(for_mongo=True))
            self.inst.insert_one_model(mdl)

        self.assertModelSetEqual(set(list(self.inst.get_queued_execodes(self.CREATOR_OID))), set(expected))
        self.assertEqual(self.inst.count_documents({}), 8)
        self.assertEqual(self.inst.count_documents({ExecodeEntryModel.CreatorOid.key: self.CREATOR_OID}), 5)
        self.assertEqual(self.inst.count_documents({ExecodeEntryModel.CreatorOid.key: self.CREATOR_OID_2}), 3)

    def test_get_list_no_exists(self):
        self.assertEqual(list(self.inst.get_queued_execodes(self.CREATOR_OID)), [])
        self.assertEqual(self.inst.count_documents({}), 0)

    def test_get_entry(self):
        execode = self.inst.generate_hex_token()
        ...
        result = self.inst.get_execode_entry(execode)
    ...
    def test_get_entry_not_exists(self):
        result = self.inst.get_execode_entry(self.inst.generate_hex_token(), Execode.REGISTER_CHANNEL)
    ...
    def test_get_entry_with_action(self):
        ...
        result = self.inst.get_execode_entry(execode, Execode.REGISTER_CHANNEL)
    ...
    def test_get_entry_action_mismatch(self):
        ...
        result = self.inst.get_execode_entry(execode, Execode.AR_ADD)
    ...
    def test_get_entry_code_mismatch(self):
        ...
        result = self.inst.get_execode_entry("A", Execode.AR_ADD)
    ...
    def test_del(self):
        ...
        self.inst.remove_execode(execode)
        self.assertEqual(self.inst.count_documents({}), 0)

    def test_del_not_exists(self):
        self.assertEqual(self.inst.count_documents({}), 0)
        self.inst.remove_execode(self.inst.generate_hex_token())
        self.assertEqual(self.inst.count_documents({}), 0)
...
class TestExecodeManagerComplete(TestModelMixin, TestDatabaseMixin):
    ...
    def test_ar_add_new(self):
        ...
        ar_inst = AutoReplyModuleManager()
        ch_inst = ChannelManager()

        self.assertEqual(ch_inst.count_documents({}), 1)
        self.assertEqual(ar_inst.count_documents({}), 1)
    ...
    def test_ar_add_invalid(self):
        ...
        self.assertEqual(ch_inst.count_documents({}), 1)
        self.assertEqual(ar_inst.count_documents({}), 0)

    def test_register_channel_new(self):
        enqueue = self.inst.enqueue_execode(ObjectId(), Execode.REGISTER_CHANNEL)
        code = enqueue.execode
        mdl = enqueue.model
    ...
    def test_register_channel_existed(self):
        ch_inst = ChannelManager()
        self.assertTrue(ch_inst.ensure_register(Platform.LINE, "U123456").success)
    ...
    def test_register_channel_platform_error(self):
        ...
        self.assertEqual(ch_inst.count_documents({}), 0)

    def _user_integrate_pre(self, reg_1, reg_2):
        ar_inst = AutoReplyModuleManager()
        u_inst = RootUserManager()

        channel_oid = ObjectId()

        if reg_1:
            creator_oid_result = u_inst.register_onplat(Platform.LINE, "U123456789")
        ...
        if reg_2:
            creator_oid_2_result = u_inst.register_onplat(Platform.LINE, "U12345678A")
            self.assertTrue(creator_oid_2_result.success)
            creator_oid_2 = creator_oid_2_result.model.id
        else:
            creator_oid_2 = ObjectId()

        ar_inst.insert_one_model(
            AutoReplyModuleModel(
                Keyword=AutoReplyContentModel(Content="A"), Responses=[AutoReplyContentModel(Content="B")],
                CreatorOid=creator_oid, ChannelOid=channel_oid))
        self.assertEqual(ar_inst.count_documents({}), 1)
        self.assertEqual(ar_inst.count_documents({AutoReplyModuleModel.CreatorOid.key: creator_oid}), 1)
    ...
    def _user_integrate_post(self, creator_oid, creator_oid_2, migrated):
        ...
        if migrated:
            self.assertEqual(ar_inst.count_documents({AutoReplyModuleModel.CreatorOid.key: creator_oid}), 0)
            self.assertEqual(ar_inst.count_documents({AutoReplyModuleModel.CreatorOid.key: creator_oid_2}), 1)
        else:
            self.assertEqual(ar_inst.count_documents({AutoReplyModuleModel.CreatorOid.key: creator_oid}), 1)
            self.assertEqual(ar_inst.count_documents({AutoReplyModuleModel.CreatorOid.key: creator_oid_2}), 0)

    def test_user_integrate(self):
        creator_oid, creator_oid_2 = self._user_integrate_pre(True, True)

        enqueue = self.inst.enqueue_execode(creator_oid, Execode.INTEGRATE_USER_DATA)

        result = self.inst.complete_execode(enqueue.execode, {param.Execode.USER_OID: str(creator_oid_2)})
    ...
    def test_user_integrate_src_eq_dest(self):
        ...
        result = self.inst.complete_execode(enqueue.execode, {param.Execode.USER_OID: str(creator_oid)})
    ...
    def test_unhandled_action(self):
        enqueue = self.inst.enqueue_execode(ObjectId(), Execode.SYS_TEST)

        result = self.inst.complete_execode(enqueue.execode, {})
