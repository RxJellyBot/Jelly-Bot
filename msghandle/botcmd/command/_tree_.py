"""Implementations to link the command nodes to be a command tree for parse."""
...
CMD_DEP = _("Dummy command for accomodating new command usage.")
DEP_TXT = _("Please add 'AR' which means 'Auto-Reply' between JC and the command code.\n------\n"
            "Example:\n"
            "**Old**\n"
            "```\n"
            "JC\n"
...
def old_arg_0(_):  # pylint: disable=missing-function-docstring
    return [HandledMessageEventText(content=DEP_TXT)]


# noinspection PyUnusedLocal
@cmd_ar_old.command_function(arg_count=1)
def old_arg_1(_, __):  # pylint: disable=missing-function-docstring
    return [HandledMessageEventText(content=DEP_TXT)]


# noinspection PyUnusedLocal
@cmd_ar_old.command_function(arg_count=2)
def old_arg_2(_, __, ___):  # pylint: disable=missing-function-docstring
    return [HandledMessageEventText(content=DEP_TXT)]
