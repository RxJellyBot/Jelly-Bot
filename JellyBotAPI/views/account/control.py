...
class AccountSettingsPageView(LoginRequiredMixin, TemplateResponseMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        update_result = RootUserManager.update_config(
            ...
            **{k.replace(keys.ParamDictFilter.Filter1, ""): v for k, v in request.POST.items()
               if k.startswith(keys.ParamDictFilter.Filter1)})

        if update_result.success:
            return simple_str_response(
                request,
                f"success/{timezone.localtime(now_utc_aware()):%m-%d %H:%M:%S (%Z)} - " + _(f"Account settings updated."))
        else:
            return simple_str_response(
                request,
                f"danger/{timezone.localtime(now_utc_aware()):%m-%d %H:%M:%S (%Z)} - " + _("Failed to update."))
